...
const VoiceInterview = () => {
  ...
  const saveTranscript = async () => {
    ...
    try {
      ...
      console.log('Sending request to save transcript with data:', JSON.stringify(requestData).substring(0, 200) + '...');
          ...
          <CardContent className="pt-6">
            <div className="mb-6 text-center">
              <div className="flex justify-center mb-4">
                <div className={`rounded-full p-6 ${active ? 'bg-green-100 animate-pulse' : 'bg-gray-100'}`}>
                  <Mic className={`h-12 w-12 ${active ? 'text-green-600' : 'text-gray-400'}`} />
                </div>
              </div>
              ...
              <Button
                onClick={toggleCall}
                className={`px-6 ${active ? 'bg-red-600 hover:bg-red-700' : 'bg-blue-600 hover:bg-blue-700'} text-white`}
              >
